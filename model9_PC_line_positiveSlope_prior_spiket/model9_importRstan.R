...
### rstan model with pitcher fatigue line and batter learning bumps

file_bsnBL = "tto9_PF_BL_line_spikedPosSlope.stan"
...
lambda = 0.2 #FIXME # weight of the spike at 0 in the slope prior
...
fit_model_bsnBL <- function(fold_num=NA) {
  ...
  data_train <- list(
    y=y_train,S=S_train,O=O_train,X=X_train,INCPT=INCPT_train,lambda=lambda,
  ...
  fit <- sampling(model_bsnBL,
                  ...
                  pars=c("linpred","alpha_incpt_raw",
                         "alpha_slope_raw_spike", "alpha_slope_raw_slab", "alpha_slope_raw",
                         "alpha_slope_spike", "alpha_slope_slab",
                         "beta_raw","eta_raw"), include=FALSE,
