...
test_tib_ubi_3a = tibble(y=numeric(0),pplower=numeric(0),ppmean=numeric(0),ppupper=numeric(0))
test_tib_ubi_3b = tibble(y=numeric(0),pplower=numeric(0),ppmean=numeric(0),ppupper=numeric(0))
...
for (fold_num in 1:10) {
  ...
  ### UBI model fit, rstan_3a final_woba
  fitU_3a <- readRDS(paste0("./job_output/fit_rstan3_comp_ubi-",fold_num,".R.rds")) #FIXME
  #plot_ubi0(fitU_3a)
  drawsU <- as_tibble(as.matrix(fitU_3a)) #FIXME
  ...
  post_pred_meansU = U_test%*%t(beta_draws) + O_test%*%t(gamma_draws) + X0_test%*%t(delta_draws) #FIXME
  ...
  test_tib_ubi_3a = bind_rows(test_tib_ubi_3a, df_ubi) #FIXME
  
  ### UBI model fit, rstan_3b BQ,PQ
  fitU_3b <- readRDS(paste0("./job_output/fit_rstan3_comp_ubi-",fold_num,".R.rds")) #FIXME
  #plot_ubi0(fitU_3b)
  drawsU <- as_tibble(as.matrix(fitU_3b)) #FIXME
  beta_draws = drawsU[,str_detect(colnames(drawsU), "^beta")]
  gamma_draws = drawsU[,str_detect(colnames(drawsU), "^gamma")]
  delta_draws = drawsU[,str_detect(colnames(drawsU), "^delta")]
  sigma_drawsU = drawsU[,str_detect(colnames(drawsU), "^sigma")]$sigma
  ...
  ppupperU = apply(post_predU, 1, function(x) quantile(x,.975))
  df_ubi = bind_cols(y_test, pplower=pplowerU, ppmean=ppmeanU, ppupper=ppupperU)
  test_tib_ubi_3b = bind_rows(test_tib_ubi_3b, df_ubi) #FIXME
}
...
rmse(test_tib_ubi_3a)
rmse(test_tib_ubi_3b)
...
covg(test_tib_ubi_3a)
covg(test_tib_ubi_3b)
...
# ratio of lengths of posterior predictive intervals of y
lengths_ubi_3a = test_tib_ubi_3a$ppupper - test_tib_ubi_3a$pplower
lengths_ubi_3b = test_tib_ubi_3b$ppupper - test_tib_ubi_3b$pplower
length_ratios = lengths_ubi_3a/lengths_ubi_3b
mean(length_ratios)
sd(length_ratios)
lp = as_tibble(length_ratios) %>% ggplot(aes(x=value)) + 
  geom_histogram(binwidth=.001) + 
  ...
  geom_vline(xintercept = 1, color="dodgerblue2") +
  xlim(c(0.9,1.2)) + 
  xlab("Ratio of UBI final_woba lengths to BQ,PQ lengths") +
  annotate(x=1,y=+Inf,label="1",vjust=2,geom="label",color="dodgerblue2") +
  annotate(x=mean(length_ratios),y=2900,label="Mean ratio of lengths",vjust=2,geom="label",color="firebrick")
lp
#ggsave("job_output/plot_ppLengthRatio.png", lp)
