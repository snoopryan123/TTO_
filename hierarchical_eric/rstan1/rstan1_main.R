...
p = (D %>% select(PIT_ID) %>% left_join(pit_indices))$pit_idx
#####
...
fit_model_hbp1 <- function(fold_num=NA, model_type="1a") {
  ...
  data_train <- list(
    K = num_categories, n = nrow(X_train), d = ncol(X_train), 
    num_pit = num_pit, num_pit_games = num_pit_games, num_bat = num_bat, num_bat_games = num_bat_games,
    p = p, pg = pg, b = b, bg = bg,
    X=X_train, y=y_train
  )
  # Train the models
