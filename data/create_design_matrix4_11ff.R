...
output_filename = "TTO_dataset_411ff.csv" #FIXME
...
get_PQBQ <- function(full=TRUE, sigma_scale=1) {
  # get Pitcher Quality
  E1a = get_PQ(E00, sigma_scale=sigma_scale)
  ...
  E1b = get_PQ(E00b, sigma_scale=sigma_scale) %>%
    rename(BQ = PQ,
           BAT_ID = PIT_ID,
  ...
  print(names(X1)[sapply(1:ncol(X1),fun <- function(i) {sum(is.na(X1[,i]))}) > 0])
  if (full) return(X1) else return(X1 %>% select(PQ,BQ))
}

X1 = get_PQBQ(full=TRUE, sigma_scale=1)
pb2 = get_PQBQ(full=FALSE, sigma_scale=sqrt(5)) %>% rename(BQ2=BQ, PQ2=PQ)
pb3 = get_PQBQ(full=FALSE, sigma_scale=sqrt(10)) %>% rename(BQ3=BQ, PQ3=PQ)
pb4 = get_PQBQ(full=FALSE, sigma_scale=5) %>% rename(BQ4=BQ, PQ4=PQ)
pb5 = get_PQBQ(full=FALSE, sigma_scale=10) %>% rename(BQ5=BQ, PQ5=PQ)
pb6 = get_PQBQ(full=FALSE, sigma_scale=sqrt(100)) %>% rename(BQ6=BQ, PQ6=PQ)

# pb7 = get_PQBQ(full=FALSE, sigma_scale=50) %>% rename(BQ7=BQ, PQ7=PQ)
# pb8 = get_PQBQ(full=FALSE, sigma_scale=sqrt(1000)) %>% rename(BQ8=BQ, PQ8=PQ) #sqrt(13)
# pb9 = get_PQBQ(full=FALSE, sigma_scale=100) %>% rename(BQ9=BQ, PQ9=PQ) #sqrt(2)
# pb10 = get_PQBQ(full=FALSE, sigma_scale=sqrt(100000)) %>% rename(BQ10=BQ, PQ10=PQ) #sqrt(3)
# pb11 = get_PQBQ(full=FALSE, sigma_scale=1000) %>% rename(BQ11=BQ, PQ11=PQ) #sqrt(1.4)

pb7 = get_PQBQ(full=FALSE, sigma_scale=10000) %>% rename(BQ7=BQ, PQ7=PQ)
pb8 = get_PQBQ(full=FALSE, sigma_scale=100000) %>% rename(BQ8=BQ, PQ8=PQ) #sqrt(13)
pb9 = get_PQBQ(full=FALSE, sigma_scale=1000000) %>% rename(BQ9=BQ, PQ9=PQ) #sqrt(2)
pb10 = get_PQBQ(full=FALSE, sigma_scale=10000000) %>% rename(BQ10=BQ, PQ10=PQ) #sqrt(3)
pb11 = get_PQBQ(full=FALSE, sigma_scale=100000000) %>% rename(BQ11=BQ, PQ11=PQ) #sqrt(1.4)
...
X2 = bind_cols(X1,pb2,pb3,pb4,pb5,pb6,pb7,pb8,pb9,pb10,pb11) %>% 
          select(-c(row_idx, 
...
X3 %>% filter(BAT_ID == "sotoj001") %>% mutate(idx = row_number()) %>% ggplot() +
  geom_line(aes(x=idx, y=std_WOBA_CURR_BAT_19), col="red") +
  geom_line(aes(x=idx, y=std_BQ), col="green") +
  geom_line(aes(x=idx, y=std_BQ2), col="green4") +
  geom_line(aes(x=idx, y=std_BQ3), col="cyan") +
  geom_line(aes(x=idx, y=std_BQ4), col="orange") +
  geom_line(aes(x=idx, y=std_BQ5), col="maroon") +
  geom_line(aes(x=idx, y=std_BQ6), col="black") +
  geom_line(aes(x=idx, y=std_BQ7), col="grey") +
  geom_line(aes(x=idx, y=std_BQ8), col="purple") +
  geom_line(aes(x=idx, y=std_BQ9), col="yellow") +
  geom_line(aes(x=idx, y=std_BQ10), col="firebrick") +
  geom_line(aes(x=idx, y=std_BQ11), col="dodgerblue") +
  ylim(c(-.05,1.5))
