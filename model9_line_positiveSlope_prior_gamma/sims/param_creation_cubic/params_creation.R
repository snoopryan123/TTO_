...
alpha_line = numeric()
for (kk in 1:7) {
  # alpha_line_k = smooth.spline((alpha_tib %>% filter(k==kk))$alpha, df=3)$y
  
  alpha_line_k = lm(a~t+I(t^2)+I(t^3), data=  tibble(t=1:27, a=(alpha_tib %>% filter(k==kk))$alpha) )
  if (kk %in% c(4,7)) { # remove selection bias from 1B, HR
    alpha_line_k$coefficients[4] = alpha_line_k$coefficients[4] + 0.000005#0.00001
  }
