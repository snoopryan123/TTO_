...
NUM_ITS = 2000 #1500 #2000 #2500 #5000
...
filename = "tto10_spl.stan"
CHANGE_DIR = if (exists("IS_SIM")) { IS_SIM } else if (exists("IS_COMP")) { IS_COMP } else { FALSE }
og_dir = getwd()
if (CHANGE_DIR) { setwd("..") }
model_stan <- stan_model(file = filename, model_name = filename)
if (CHANGE_DIR) { setwd(og_dir) }

fit_model_spl <- function(fold_num=NA) {
  ...
  data_train <- list(
    y=y_train,S=S_train,X=X_train,INCPT=INCPT_train,
    p_s=ncol(S_train),p_x=ncol(X_train),
  ...
  fit <- sampling(model_stan,
                  ...
                  pars=c(
                    "linpred","alpha_raw","eta_raw","alpha_incpt_raw","alpha"
                  ), include=FALSE,
