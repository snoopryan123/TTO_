...
# create PITCH_CT column
D <- D %>% group_by(GAME_ID, PIT_ID) %>%
  mutate(PITCH_CT = lag(PITCH_COUNT_CUMU, default=0)) %>%
  ungroup()
# D %>% select(GAME_ID, PIT_ID, BAT_ID, INNING, PITCH_COUNT_CUMU, PITCH_CT) ## check
...
### LINE data matrix
SPL = matrix(D$PITCH_CT, ncol=1) ### linear in PITCH COUNT
