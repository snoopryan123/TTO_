...
D1 <- D %>% mutate(sb = std_WOBA_FINAL_BAT_19,
                   sb2 = std_WOBA_FINAL_BAT_19**2,
                   sb3 = std_WOBA_FINAL_BAT_19**3,
                   sp = std_WOBA_FINAL_PIT_19,
                   sp2 = std_WOBA_FINAL_PIT_19**2,
                   sp3 = std_WOBA_FINAL_PIT_19**3)
X <- bind_cols(BATTER_SEQ_dummies, D1 %>% select(sb, sb2, sb3, sp, sp2, sp3,
...
plot1 = A4 %>% 
  ...
  labs(title = paste0(OUTPUT_FILE, " cubic batter & pitcher quality")) +
