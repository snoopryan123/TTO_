...
bsn_fit_to_posterior_probs <- function(S_test,X_test,fit) {
  ...
  c_draws = draws[,str_detect(colnames(draws), "^c")]
  
  linpred = S_test%*%t(alpha_draws) + X_test%*%t(eta_draws)
  
  c1_minus_mu = matrix(c_draws[,1], nrow=nrow(linpred), ncol=nrow(c_draws), byrow = TRUE) - linpred
  c2_minus_mu = matrix(c_draws[,2], nrow=nrow(linpred), ncol=nrow(c_draws), byrow = TRUE) - linpred
  c3_minus_mu = matrix(c_draws[,3], nrow=nrow(linpred), ncol=nrow(c_draws), byrow = TRUE) - linpred
  c4_minus_mu = matrix(c_draws[,4], nrow=nrow(linpred), ncol=nrow(c_draws), byrow = TRUE) - linpred
  c5_minus_mu = matrix(c_draws[,5], nrow=nrow(linpred), ncol=nrow(c_draws), byrow = TRUE) - linpred
  c6_minus_mu = matrix(c_draws[,6], nrow=nrow(linpred), ncol=nrow(c_draws), byrow = TRUE) - linpred
  
  p1 = pnorm(c1_minus_mu)
  p2 = pnorm(c2_minus_mu) - pnorm(c1_minus_mu)
  p3 = pnorm(c3_minus_mu) - pnorm(c2_minus_mu)
  p4 = pnorm(c4_minus_mu) - pnorm(c3_minus_mu)
  p5 = pnorm(c5_minus_mu) - pnorm(c4_minus_mu)
  p6 = pnorm(c6_minus_mu) - pnorm(c5_minus_mu)
  p7 = 1 - p1 - p2 - p3 - p4 - p5 - p6 
  
  # p1[1:10,1:10]
  # p1[1,1]+p2[1,1]+p3[1,1]+p4[1,1]+p5[1,1]+p6[1,1]+p7[1,1]
  probs = list(p1,p2,p3,p4,p5,p6,p7)
  probs
}
...
fit <- readRDS("job_output/fit_rstan7-1.R.rds")
draws <- as.matrix(fit)

test_rows = which(folds == 6)
S_test = S[test_rows,]
X_test = X[test_rows,]
y_test = y[test_rows,]
